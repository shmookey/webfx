...
fn vert_main([[location(0)]] position: vec3<f32>,
             [[location(1)]] latlon:   vec2<f32>,
             [[location(2)]] normal:   vec3<f32>,
             [[location(3)]] uv:       vec2<f32>) -> VertexOutput {
...
fn frag_main([[builtin(position)]] pos:      vec4<f32>,
  ...
  output.colour = vec4<f32>((diffuse + ambient)*fragParams.colour.rgb , 1.0);
  //var d = min(uv[0], uv[1]);
  //if(d > 0.07) { output.colour = vec4<f32>(0.0,0.0,0.0,0.0); }
  return output;
}
